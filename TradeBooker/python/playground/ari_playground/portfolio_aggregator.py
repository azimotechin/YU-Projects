    ...
    def _update_position(self, key: str):
        try:
            ...
            if not hash_data:
                ...
                return
            # Only process if full hash exists
            if len(hash_data) < 6:
            ...
            if trade_type == "sell":
                ...
                # If no shares are held, ignore the trade and delete the invalid hash
                if current_shares <= 0:
                    logger.warning(
                        f"Position Aggregator: No shares of {ticker} for {account} to sell. Ignoring and deleting bad trade hash.")
                    # Delete the bad trade hash to prevent other services from seeing it.
