        ...
        while True:
            try:
                ...
                for stream, entries in messages:
                    #potentially remove the pipeline logic and uncomment out the self. ... lines. also, still need to make sure that this class not only works when there is one consumer, but even when there are multiple consumers all updating the same position hash simultaneously, when things can be happening out of order or at same time etc
                    pipe = self.redis.pipeline() #potentially add transaction=false as a param
                    for msg_id, fields in entries:
                        try:
                            # Expecting a single 'trade_string' field now                         
                            trade = Trade.create_from_full_input(fields["trade_string"])
                            pipe.hincrby(self.position_hash, f"{trade.account_id}/{trade.ticker}", trade.quantity if trade.trade_type == "buy" else -trade.quantity)
                            ...
                            pipe.xack(self.stream_key, self.group, msg_id) #still acknowledge the message even if processing fails, avoiding it being attempted again and again
                    pipe.execute()
            except Exception as e:
