...
# Add parent directory to sys.path BEFORE importing app_utils modules
...
def get_redis_connection():
    ...
    try:
        ...
        st.error(f"❌ Could not connect to Redis Sentinel: {e}")
        return None
...
def stock_data_tab():
    st.write("This tab will display stock data in the future.")
    # Placeholder for future stock data functionality
    st.info("Stock data functionality is coming soon! Stay tuned.")
    # You can implement stock data fetching and display logic here later.

def set_stock_filters_main_tab(r):
    # Get all stocks for options
    stocks = set()
    for key in r.scan_iter(match="stock:*", count=5000):
        try:
    ...
    # Dropdown select menu for stocks (single selection)
    selected_stock = st.selectbox(
        "Select Stock",
        options=stocks,
        key="stock_selectbox_main"
    )

    start_date = st.date_input("Start Date", value=pd.to_datetime('1900-01-01'), key="stock_start_date_main")
    end_date = st.date_input("End Date", value=pd.to_datetime('2200-12-31'), key="stock_end_date_main")

    # Save filters to session state
    st.session_state["stock_filters"] = {
        "stocks": [selected_stock] if selected_stock else [],
        "start_date": pd.to_datetime(start_date),
        "end_date": pd.to_datetime(end_date)
    }

@st.fragment(run_every="5s")
def display_stock_table(r):
    stocks = []
    filters = st.session_state.get("stock_filters", {})
    selected_stocks = filters.get("stocks", [])
    for key in r.scan_iter(match="stock:*", count=5000):
        try:
            stock_data = r.hgetall(key)
            ticker = stock_data.get("ticker", "")
            if ticker and ticker in selected_stocks:
    ...
    if stocks:
        ...
        display_cols = ['ticker', 'price', 'volume', 'market_cap']
        existing_cols = [col for col in display_cols if col in df.columns]
        st.dataframe(df[existing_cols])
    else:
        st.warning("No stock data found for the selected criteria.")
...
def stock_data_tab(r):
    ...
    with stock_tabs[0]:
        ...
        set_stock_filters_main_tab(r)
        display_stock_table(r)
    with stock_tabs[1]:
        st.subheader("Historical Stock Data")
        set_stock_filters_main_tab(r)
        st.write(f"#### Showing historical data for {len(st.session_state.stock_filters['stocks'])} stock(s) from {st.session_state.stock_filters['start_date']} to {st.session_state.stock_filters['end_date']}")
        st.info("This tab will display historical stock data in the future.")

# In your main() function, replace the stock data tab section with:
    
def main():
    st.set_page_config(page_title="Trade Viewer", layout="wide")
    r = get_redis_connection()
    trade_manager = TradeManager(redis_client=r)
