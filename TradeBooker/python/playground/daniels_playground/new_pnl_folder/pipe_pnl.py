...
class PnLCalculator:
    ...
    def listen_and_calculate(self):#Listens for trades coming into the stream and calculates pnl, runs continously
        logger.info("Starting PnL Calculator - listening for trades")
        ...
        while True:
            try:
                messages = self.redis.xreadgroup(
                    ...
                    count=1000, #Process 1000 trades at a time
                ...
                # Create pipeline for batching all write operations
                ...
                for stream, entries in messages:
                    for msg_id, fields in entries:
                        try:
                            ...
                            # Fifo Pnl calculation with pipeline
                            self.process_trade_fifo(trade, pipe)
                            ...
                            # Every 1000 trades, log overall rate from first trade this consumer has bookd through now
                            if self.booked % 1000 == 0:
                ...
                # Add acknowledgments to pipeline
                ...
                try:
                    pipe.execute()
                    logger.info(f"Successfully executed pipeline with {len(processed_msg_ids)} trades")
                except Exception as e:
    ...
    def process_trade_fifo(self, trade:Trade, pipe=None): #fucntion that processes accroding to whether the trade is a buy or sell

        if trade.trade_type == "buy":
            self._process_buy_fifo(trade, pipe)
        elif trade.trade_type == "sell":
            self._process_sell_fifo(trade, pipe)
        else:
            logger.error(f"Unknown trade type {trade.trade_type}")

    def _process_buy_fifo(self, trade:Trade, pipe=None):
        ...
        if pipe is None:
            ...
            # Add save operation to pipeline
            lots_key = self._get_lots_key(trade.account_id, trade.ticker)
            lots_json = json.dumps(existing_lots)
            pipe.set(lots_key, lots_json)
    ...
    def _process_sell_fifo(self, trade:Trade, pipe=None):
        ...
        for i in reversed(lots_to_remove): # test for this
            del existing_lots[i]
        ...
        if pipe is None:
            ...
            # Add lots save operation to pipeline
            lots_key = self._get_lots_key(trade.account_id, trade.ticker)
            lots_json = json.dumps(existing_lots)
            pipe.set(lots_key, lots_json)
        ...
        if total_realized_pnl != 0:
            ...
            if pipe is None:
                ...
                self.calculate_unrealized_pnl_single(trade.account_id,trade.ticker)
            else:
                pipe.hincrbyfloat(self.realized_pnl_by_position_hash, position_key, total_realized_pnl)#does this method only update or add to redis if it doesnt exist, make sre it adds to the previous realized pnl if it is there already
                logger.info(f"Realized PnL for {position_key}: ${total_realized_pnl:.2f}")
                self.calculate_unrealized_pnl_single(trade.account_id,trade.ticker, existing_lots, pipe)
    ...
    def calculate_unrealized_pnl_single(self, account_id: str, ticker: str, current_lots: list = None, pipe = None) -> float:
        ...
        if not lots:
            if pipe is not None:
                # If no lots remaining, set unrealized PnL to 0
                position_key = f"{account_id}/{ticker}"
                pipe.hset(self.unrealized_pnl_by_position_hash, position_key, 0.0)
        ...
        if pipe is not None:
            # Add unrealized PnL update to pipeline
            position_key = f"{account_id}/{ticker}"
            pipe.hset(self.unrealized_pnl_by_position_hash, position_key, total_unrealized_pnl)
