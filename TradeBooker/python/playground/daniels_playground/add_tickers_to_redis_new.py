...
def get_ticker_history(ticker: str):
    """
    Fetch historical price data for a ticker across multiple periods.
    Returns a dictionary with period as key and single price as value.
    """
    try:
        ...
        period_to_days = {
            ...
            '3mo': 66,  # ~3 months of trading days
        ...
        for period in HISTORY_PERIODS:
            try:
                if period == 'ytd':
                    # For YTD, get data from Jan 1st of current year
                    ...
                    # Get enough data to find the price from X days ago
                    hist = ticker_obj.history(period="2y")  # Get 2 years to ensure we have enough data
                
                if not hist.empty:
                    if period == 'ytd':
                        # For YTD, get the first available price of the year
                        ...
                        if len(hist) > days_back:
                            ...
                            # If not enough data, use the earliest available price
                            price = round(float(hist['Close'].iloc[0]), 2)
        ...
        return {period: None for period in HISTORY_PERIODS}


def update_ticker_history(ticker: str, r: redis.Redis):
    """
    Update the historical price data for a single ticker in Redis.
    Stores each period as a separate key: <Ticker>:1d, <Ticker>:5d, etc.
    """
    try:
        history_data = get_ticker_history(ticker)
...
def update_all_ticker_histories(tickers: List[str], r: redis.Redis):
    ...
    This runs once per day and updates ALL tickers.
    """
    ...
    for batch_num in range(total_batches):
        ...
        # Process batch with reduced delay for speed
        batch_success = 0
        for ticker in batch:
            try:
                if update_ticker_history(ticker, r):
                    batch_success += 1
                    success_count += 1
                time.sleep(0.05)  # Small delay to respect API limits
            except Exception as e:
    ...
    print(f"\n✓ History update completed: {success_count}/{len(tickers)} tickers updated successfully")


def check_and_create_missing_histories(tickers: List[str], r: redis.Redis):
    """
    Check if price histories exist for tickers, create them if missing.
    ...
    if missing_tickers:
        ...
        for i, ticker in enumerate(missing_tickers, 1):
            try:
                ...
                # Progress indicator
                if i % 50 == 0 or i == len(missing_tickers):
                    print(f"\rCreating histories: {i}/{len(missing_tickers)} processed ({success_count} successful)", end='', flush=True)
                
                # Small delay to be respectful to the API
                time.sleep(0.1)
                
            except Exception as e:
                print(f"Error creating history for {ticker}: {e}")
        
        print(f"\n✓ Created {success_count}/{len(missing_tickers)} missing price histories")
    else:
