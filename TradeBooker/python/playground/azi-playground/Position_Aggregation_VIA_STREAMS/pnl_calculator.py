...
class PnLCalculator:
    ...
    def _get_lots_key(self, account_id: str,
                      ticker: str) -> str:  # generates a redis key in the following format 'lots:alice/AAPL'
        return f"{self.lots_key_prefix}{account_id}/{ticker}"

    def _get_lots(self, account_id: str,
                  ticker: str) -> list:  # returns a list of all lots(trades) for the user and ticker
    ...
    def _save_lots(self, account_id: str, ticker: str,
                   lots: list):  # Saves lots and stores in into json for the lot key
        ...
        while True:
            try:
                messages = self.redis.xreadgroup(
                    ...
                    count=1000,  # Process 10 trades at a time
    ...
    def process_trade_fifo(self,
                           trade: Trade):  # fucntion that processes accroding to whether the trade is a buy or sell
    ...
    def _process_sell_fifo(self, trade: Trade):
        ...
        for i, lot in enumerate(existing_lots):
            ...
            if lot[
                "quantity"] <= remaining_to_sell:  # this means sell all shares in the lot and since it is fully consumed, add it the lots_to_remove list
                realized_pnl_from_lot = (trade.price - lot["price"]) * lot[
                    "quantity"]  # calculating realized pnl by price sold - price bought for multiplied by how many shares sold
        ...
        if total_realized_pnl != 0:
            ...
            self.redis.hincrbyfloat(self.realized_pnl_by_position_hash, position_key,
                                    total_realized_pnl)  # does this method only update or add to redis if it doesnt exist, make sre it adds to the previous realized pnl if it is there already
